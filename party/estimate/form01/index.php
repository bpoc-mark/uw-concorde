...
/*----------------------------------------------------------------------------------------------------------------------------------------------------
	...
	フォームメール - sformmmail2（UWカスタマイズ版）　(c)Sapphirus.Biz

　－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－

　・入力必須の場合亜はname値の後に「_s」を追加して下さい。
　　但し、確認画面・メールテンプレート内は「_s」を外した状態で設定して下さい。

　・input type="radio" / input type="checkbox"かつ入力必須の場合は各inputの前にtype="hidden"で同情報を追加して下さい。
　　必須項目でない場合は、hiddenの設定は不要です。


　■要カスタム箇所
　　「printSubmit」… $submit = <<< EOD内の戻る・送信ボタンを状況に応じて変更下さい
...
,	'form'			=> 'sfm01_form.php'	 		// 入力フォーム用
,	'confirm'		=> 'sfm02_confirm.php' 		// 確認画面用
,	'completion'	=> 'sfm03_completion.php' 	// 送信完了画面
...
<p class="btn_back"><input type="button" onclick="history.back(); return false;" value="入力画面に戻る"/></p>
EOD;
	} else {

// 項目が正しい場合のHTML出力
$submit = <<< EOD
<p>入力内容が正しければ、<br class="sp">「送信する」ボタンを押して下さい。</p>
<input type="hidden" name="mode" id="mode" value="SEND" />
<input name="autoReply" type="hidden" value="1" />
<ul class="cf">
<li class="send"><input type="submit" value="送信する" id="submit"/></li>
<li class="btn_back"><input type="button" onclick="history.back(); return false;" value="入力画面に戻る"/></li>
</ul>
EOD;
}
return $submit;
...
class SbFormMailClass
{
	...
	{
		$this->cfg = $cfg;
		if (!extension_loaded('mbstring')) { $this->exitErr('マルチバイト関数が利用できません。'); }
		mb_language('ja');
		...
		// 受け取りアドレスの設定をチェック
		if (!isset($cfg['mailTo'][0])) { $this->exitErr('受取先メールアドレスが設定されてません。'); }

		// モードによる条件分岐
		$mode = '';
		if (isset($_POST['mode'])) { $mode = $_POST['mode']; }
		if (isset($_POST['back'])) { $mode = 'BACK'; }
		...
		switch ($mode) {
			...
			case 'SEND':
				...
				session_start();
				if (!isset($_SESSION['SFM'])) { header("Location: ./"); exit; }
				...
				if(isset($_SESSION['SFM']['email']) && is_file($cfg['temp_html']['reply'])) {
			...
			case 'CONFIRM':
				...
				foreach ($_POST as $key => $value) {
					...
					if (is_array($value)) {
						...
						if ($value == '__Error_Marge_Data__') { $error = 1; }
					}
					if (!$cfg['ill_slash']) {
